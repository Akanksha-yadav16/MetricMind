const API = (
  process.env.NEXT_PUBLIC_API_URL || "http://192.168.1.129:8000"
).replace(/\/$/, "");
...
async function fetchJSON(url, options = {}) {
  ...
  try {
    ...
    if (error.name === "AbortError") {
      throw new Error(
        "The analytics service took too long to respond. Please try again."
      );
    }
...
export const getTotalOrders = () =>
  fetchAPI("/total-orders");

export const getTotalCustomers = () =>
  fetchAPI("/total-customers");

export const getTotalRevenue = () =>
  fetchAPI("/total-revenue");

export const getAverageOrderValue = () =>
  fetchAPI("/average-order-value");

export const getTopSellingProduct = () =>
  fetchAPI("/top-selling-product");

export const getTopCustomer = () =>
  fetchAPI("/top-customer");

export const getRevenueByProduct = () =>
  fetchAPI("/revenue-by-product");

export const getProductReport = () =>
  fetchAPI("/product-report");